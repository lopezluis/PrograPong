...
#include <curses.h>         // por clear, refresh, getch, mvaddstr
...
void mostrar_tabla_puntajes(Juego *juego)
{
	...
	{
		mvaddstr(3, (juego->anchoTablero - 40) / 2, "La tabla de mejores puntajes est\xA0 vac\xA1\x61.");
        mvaddstr(5, (juego->anchoTablero - 45) / 2, "Presione una tecla para ir al men\xA3 principal.");
	}
	else
	{
		mvaddstr(3, 0, "Mejores puntajes:");
		// Saco la declaración de i del while para que quede locar a este ámbito y utilizarla en Presione una tecla...
		unsigned int i = 0;
        ...
        mvaddstr(i + 6, (juego->anchoTablero - 45) / 2, "Presione una tecla para ir al men\xA3 principal.");
	}
...
void agregar_nuevo_record(Juego *juego, unsigned int puntajeObtenido)
{
	...
	mvaddstr(juego->altoTablero + 4, 40, "Nombre hasta 9 car\xA0\x63teres:            ");
	refresh();
	// Contexto para acotar la variable nombre e k
    {
		...
		// Acotamos continuar_ingresando
        {
            ...
            while(continuar_ingresando)
            {
                ...
                {
                case '\x08': // Backspace, tener en cuenta que en Linux, este valor es '\x07'
                    if(k > 0)
                    {
                        k--;
                        nombre[k] = ' ';
