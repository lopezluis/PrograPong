...
void juego_correr(Juego *juego)
{
	...
	while(1)
	{
            ...
            if(juego->modo == AVENTURA)
            {
                ...
                if(juego->nivel != aux)
                {
                    ...
                    if(juego->nivel == 11)
                    {
                        ...
                        mvaddstr(juego->altoTablero + 3, 40, "\xAD\x46\x65licidades! Ah ganado.                ");
                        // Recordar que cuando escribimos en juego->altoTablero + 4, debemos evitar la escritura de un caracter en la última fila y la última columna, para evitar el scroll de la pantalla completa
                        mvaddstr(juego->altoTablero + 4, 40, "Presione una tecla para iniciar.       ");
                        refresh();
                        while(getch() == ERR);
		...
		// Actualizar e imprimir pantalla: estas dos funciones no hacen falta gracias a la utilización de la librería ncurses/PDCurses, además el término "imprimir" es inadecuado, no se utiliza una impresora, sino una pantalla, mejor utilizar "visualizar"
//		juego_actualizar_pantalla(juego);
//		juego_imprimir(juego);

		// Verificación del final de la partida según modo de juego. Esto se hace en los lugares correspondientes
		/* ... */

		// Esperar FRAME_MS unidad mínima de tiempo:
		// Esta operación no hace falta, dado que la temporización del juego la hago con frecuencias diferentes para cada etapa en la ejecución
		// TODO: averiguar si es necesario especificar un Sleep obligatoriamente con el objetivo de bajar la temperatura del núcleo del procesador que ejecuta este código. Quizás es necesario que este desarrollo deba ser multithreading (multihilo en español).
//		Sleep(FRAME_MS);
